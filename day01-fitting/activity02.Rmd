...
Answer: I would recommend using a scatterplot, where each data point represents a country and the X and Y axis are pf_expression_control and pf_score, respectively.
...
3. What type of plot would you use to display the relationship between the personal freedom score, `pf_score`, and the political pressures and controls on media content index,`pf_expression_control`?
  ...
  Answer: I would use a scattterplot
...
ggplot(hfi_2016, aes(x = pf_expression_control, y = pf_score)) +
  geom_point() +
  labs(title = "Personal Freedom Score vs. PF Expression Control",
       x = "PF Expression Control",
       y = "PF Score",
       caption = "Source: Open Intro Human Freedom Index https://www.openintro.org/data/index.php?data=hfi") +
  theme_minimal()
...
Answer: Yes, there appears to be a linear positive relationship between pf_score and pf_expression_control.
...
Answer: It's not terrible for a model, since it is above 0.5, but closer to 1.0 would be better.
...
```{r train-fit-lm}
test_aug <- augment(hfi_2016_train, new_data = hfi_2016_test)
test_aug
```
...
```{r augment}
train_aug <- augment(hfi_2016_train, new_data = hfi_2016_test)
