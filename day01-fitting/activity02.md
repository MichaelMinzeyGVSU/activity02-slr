...
Each row represents a different country and year. The dimensions
...
## I would recommend using a scatterplot, where each data point represents a country and the X and Y axis are pf_expression_control and pf_score, respectively.
...
ggplot(hfi_2016, aes(x = pf_score)) +
  ...
  labs(title = "Distribution of Personal Freedom Score in 2016",
       x = "PF Score",
       y = "Frequency",
       caption = "Source: Open Intro Human Freedom Index https://www.openintro.org/data/index.php?data=hfi") +
  theme_minimal()
```
...
ggplot(hfi_2016, aes(x = pf_expression_control)) +
  geom_histogram(binwidth = 0.25, fill = "skyblue", color = "black") +
  labs(title = "Distribution of Personal Freedom Expression Control in 2016",
...
PF Score: Center: Around 7 Spread: Shape: Outliers:

PF Expression Control: Center: Around 5 Spread: Shape: Outliers:
